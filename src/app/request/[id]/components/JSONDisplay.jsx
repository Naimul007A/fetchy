...
export const JSONDisplay = ({ raw, nested = true, className }) => {
    const obj = JSON.parse(raw);

    return (
        <div className={cn(`p-4 max-h-[500px] overflow-auto rounded-md border shadow-sm text-sm font-mono`, className)}>
            <Table className="w-full table-auto border-collapse">
                <TableBody>
                    {Array.isArray(obj) ? (
                        obj.map((value, idx) => {
                            return <TableRow key={idx} className="hover:bg-transparent">
                                <TableCell className="break-all text-gray-300">
                                    {nested && typeof value === 'object'
                                        ? <JSONDisplay className={`max-h-full`} raw={JSON.stringify(value)} nested />
                                        : renderValue({ value })}
