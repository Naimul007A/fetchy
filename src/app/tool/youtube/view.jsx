...
import { enableYoutube } from "@/conf";

const YoutubeDownloaderView = () => {
    return (
        <Root>
            <Card className={`w-full m-2 sm:m-5 lg:m-10 bg-card/10 backdrop-blur-[7px] ${enableYoutube ? "min-h-[calc(100vh-16px)] sm:min-h-[calc(100vh-40px)] lg:min-h-[calc(100vh-80px)]" : "min-h-auto"}`}>
                ...
                <CardContent className={`flex ${enableYoutube ? "min-h-[240px]" : "min-h-auto"}`}>
                    {!enableYoutube && <div className="w-full h-full flex flex-col items-center justify-center space-y-4 p-6 bg-zinc-900/30 rounded-lg border border-dashed border-zinc-700">
