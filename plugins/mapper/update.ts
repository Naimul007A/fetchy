...
export async function updateToolsMap(filePaths: string[]) {
  ...
  for (const file of filePaths) {
    try {
      ...
      imports.push(
        `import { frontmatter as ${importPath.split("/").at(-2)}_fm } from "${removeExt(importPath)}"`
      );
      items.push(
        `{path:"${file}",meta:"${metaPath}",${/title\s*:\s*(?:"([^"]*)"|'([^']*)'|([^,\n}]*))/.test(match[1]) ? `` : `title:"${file.split(path.sep).at(-2)}",`}frontmatter:${file.split("/").at(-2)}_fm,url:"${parseNextRoute(file)}"}`
      );
    } catch (err) {
